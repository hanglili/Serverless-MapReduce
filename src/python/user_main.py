...
config_pipeline_3 = {
    "outputSourceType": "s3",
    "outputSource": "serverless-mapreduce-storage-pipeline-3",
    "outputPrefix": "output"
}

serverless_mr = ServerlessMR()
pipeline1 = serverless_mr.config(config_pipeline_1).map_shuffle(extract_data_s3, partition).\
    reduce(reduce_function).finish()

pipeline2 = serverless_mr.config(config_pipeline_2).map_shuffle(extract_data_dynamo_db, partition).\
    reduce(reduce_function).finish()

pipeline3 = serverless_mr.config(config_pipeline_3).merge_map([pipeline1, pipeline2]).\
    map_shuffle(identity_function, partition).reduce(reduce_function).run()
