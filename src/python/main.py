...
from utils import data_encoding, access_s3
...
def register_and_invoke_driver():
    ...
    config = json.loads(open(Driver.DRIVER_CONFIG_PATH, 'r').read())
    # init
    ...
    data_encoding.zip_lambda(config["mapper"]["name"], config["mapper"]["zip"])
    data_encoding.zip_lambda(config["reducer"]["name"], config["reducer"]["zip"])
    data_encoding.zip_lambda(config["reducerCoordinator"]["name"], config["reducerCoordinator"]["zip"])

    data_encoding.zip_driver_lambda(config["driver"]["name"], config["driver"]["zip"])
    ...
    l_driver.update_code_or_create_on_noexist()
